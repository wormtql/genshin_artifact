...
pub struct PhysicalDamageTargetFunction {
    pub t: usize,
}

impl TargetFunctionMetaTrait for PhysicalDamageTargetFunction {
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        let t = match *config {
            TargetFunctionConfig::PhysicalDamage { t } => t,
            _ => 0
        };

        Box::new(PhysicalDamageTargetFunction {
            t
        })
    }
}
...
impl TargetFunction for PhysicalDamageTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, _character: &Character<SimpleAttributeGraph2>, _weapon: &Weapon<SimpleAttributeGraph2>, _artifacts: &[&Artifact], _enemy: &Enemy) -> f64 {
        ...
        let critical_damage = attribute.get_value(AttributeName::CriticalDamageBase) + attribute.get_value(AttributeName::CriticalDamagePhysical);

        let bonus = attribute.get_value(AttributeName::BonusBase) + attribute.get_value(AttributeName::BonusPhysical);
