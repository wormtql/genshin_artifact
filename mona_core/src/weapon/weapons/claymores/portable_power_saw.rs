...
use crate::weapon::weapons::the_dockhands_assistant::TheDockhandsAssistantEffect;

pub struct PortablePowerSaw;

impl WeaponTrait for PortablePowerSaw {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        name: WeaponName::PortablePowerSaw,
        internal_name: "Claymore_Mechanic",
        weapon_type: WeaponType::Claymore,
        weapon_sub_stat: Some(WeaponSubStatFamily::HP120),
    ...
    fn get_effect<A: Attribute>(character: &CharacterCommonData, config: &WeaponConfig) -> Option<Box<dyn WeaponEffect<A>>> {
        let stack = match *config {
            WeaponConfig::PortablePowerSaw { stack } => stack,
            _ => 0.0
        };
