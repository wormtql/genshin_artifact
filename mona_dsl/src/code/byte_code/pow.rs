...
pub struct ByteCodePow;

impl ByteCode for ByteCodePow {
    ...
    fn execute(&self, env: &mut MonaEnv) -> Result<(), RuntimeError> {
        ...
        let value1 = match &op1.borrow().data {
            ...
            _ => return Err(RuntimeError::new(RuntimeErrorEnum::TypeError, &format!("expecting number, found {}", op1.borrow().get_type())))
        };
